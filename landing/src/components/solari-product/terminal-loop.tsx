...
/**
 * The "Everything your agent needs" terminal on /sandboxes. The live section
 * shows 0.55 % of the viewport changing inside a 440x310 region — a terminal
 * that types itself out while the state chips beneath it advance
 * (`docs/research/INTERACTION_PATTERNS.md`, section 4).
 *
 * The script, the state chips and the frame title are all props so the same
 * loop can be re-scripted elsewhere (see /kleeto); the defaults are the Solari
 * script, so `<StatefulTerminal />` renders exactly as before.
 */
