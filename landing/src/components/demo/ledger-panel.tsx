 ...
 *
 * Payments only. Files the agent hands over have their own place — putting them here made the
 * money column something you had to read past to find out what you were charged.
 *
...
const VERB: Record<Exclude<RunEvent["kind"], "file">, string> = {
  topup: "paid in",
  rent: "took",
  return: "gave back",
};
...
export function LedgerPanel({ events, phase }: { events: RunEvent[]; phase: string }) {
  ...
  const spend = events.filter((e) => e.kind !== "file");
        ...
        <div className="flex h-full items-center justify-center p-6">
          <p className="max-w-[34ch] text-center text-[12.5px] leading-[1.6] text-balance text-white/30">
            {phase === "working"
              ...
              : "Nothing has been paid yet. The agent answers Kleeto’s 402 from its own wallet once you approve its plan. Every payment lands here with a link to the transaction."}
          </p>
        </div>
      ) : (
              ...
              >
                ...
                <span className="min-w-0">
                  <span className="block text-[12.5px] leading-[1.5] text-white/70">
                    <span className="text-white/40">{VERB[e.kind as Exclude<RunEvent["kind"], "file">]} </span>
                    {e.kind === "topup"
